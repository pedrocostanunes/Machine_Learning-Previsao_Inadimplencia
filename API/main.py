...
def previsao_modelo(idade, salario, situacao_moradia, tempo_trabalho, 
    ...
    return {'result': float(modelo.predict(dados_transformados)[0]),
            'probability_0': float(modelo.predict_proba(dados_transformados).tolist()[0][0]),
            'probability_1': float(modelo.predict_proba(dados_transformados).tolist()[0][1])}
